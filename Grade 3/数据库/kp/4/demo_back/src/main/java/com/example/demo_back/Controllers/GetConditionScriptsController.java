...
@Controller
public class GetConditionScriptsController {
    @Autowired
    ...
    @ResponseBody
    @PostMapping("/getconditionscript")
    public List<ConditionalScript> main(HttpServletRequest request){
        String user = request.getParameter("id");
        List<Script> l = accountService.findAccountById(Integer.valueOf(user)).getScripts();
        List<ConditionalScript> result = new ArrayList<>();
        for(Script s : l){
            if(s.getConditionalScript()!=null){
                result.add(s.getConditionalScript());
            }
        }
        return result;
    }
