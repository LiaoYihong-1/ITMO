...
def Bessel(t: float, dots: []):
    ...
    while times > 0:
        try:
            if n == 0:
                result = result + (dots[mid].y + dots[mid - 1].y) / 2 + (t - 0.5) * get_det_y(dots, mid, 1)
            else:
                a = a * (t - n) * (t + n - 1)
                a_1 = (get_det_y(dots, mid - n, 2 * n) + get_det_y(dots, mid - n + 1, 2 * n)) / (
                            math.factorial(2 * n) * 2)
                result = result + a * a_1
                b = b * (t - n) * (t + n - 1)
                b_1 = (t - 0.5) * get_det_y(dots, mid - n, 2 * n + 1) / math.factorial(2 * n + 1)
                result = result + b * b_1
            n = n + 1
            times = times - 1
        except IndexError:
...
def dot_table(times: int):
    ...
    plt.plot(x_range, y_range_L, "red")
    plt.plot(x_range, y_range_N, "black")
    plt.plot(x_range, y_range_S, "purple")
